...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
	...
	if ((self = [super initWithStyle:style reuseIdentifier:reuseIdentifier])) {
		...
		[rightTextField setFont:FONT_MAIN];
...
- (void)layoutSubviews {
	...
	if (leftLabel.text != nil) {
		...
		rightTextField.frame = CGRectMake(origFrame.origin.x+105, origFrame.origin.y, origFrame.size.width-120, origFrame.size.height-4);
	} else {
		...
		rightTextField.frame = CGRectMake(origFrame.origin.x+imageWidth+10, origFrame.origin.y, origFrame.size.width-imageWidth-20, origFrame.size.height-4);
	}
}
